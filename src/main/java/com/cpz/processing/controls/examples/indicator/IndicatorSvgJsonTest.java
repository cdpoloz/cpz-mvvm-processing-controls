...
/**
 * JSON SVG visual example for the non-interactive indicator control.
 *
 * @author CPZ
 */
public class IndicatorSvgJsonTest extends PApplet {
    private static final String INDICATOR_CONFIG_PATH = "data" + File.separator + "config" + File.separator + "indicator-svg.json";
    ...
    public void setup() {
        ...
        this.runtimeIndicator = Util.getControl(this.controls, "indSvgRuntimeJson", Indicator.class);
    ...
    private void updateRuntimeIndicator() {
        ...
        this.runtimeIndicator.setTooltipText(runtimeOn ? "JSON SVG runtime indicator on" : "JSON SVG runtime indicator off");
        this.tooltips.refresh();
    }

    private void drawLabels() {
        fill(220);
        text("JSON SVG off", 142.0F, 166.0F);
        text("JSON SVG on", 310.0F, 166.0F);
        text("JSON SVG runtime", 478.0F, 166.0F);
        fill(150);
        text("Loaded from data/config/indicator-svg.json", width * 0.5F, 226.0F);
    }
