...
public final class DefaultTextRenderer implements LabelRenderer {
   ...
   */
   public void render(PApplet sketch, float x, float y, float width, float height, LabelRenderStyle renderStyle) {
      TypographySupport.prepareStyleScope(sketch, renderStyle.typography().font());
      sketch.pushStyle();
      try {
         TypographySupport.apply(sketch, renderStyle.typography().font(), renderStyle.typography().textSize());
